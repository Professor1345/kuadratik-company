...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  if (isLoading) return <><Header onSearch={() => {}} /><LoadingSpinner /></>;
  if (error) return <><Header onSearch={() => {}} /><ErrorMessage message="Failed to load product" /></>;
  if (!product) return <><Header onSearch={() => {}} /><ErrorMessage message="Product not found" /></>;
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="flex items-center space-x-4 mb-6">
                <span className="text-3xl font-bold text-[#22A24F]">${product.price.toFixed(2)}</span>
                <span className="text-xl text-gray-500 line-through">${(product.price * 1.2).toFixed(2)}</span>
                <span className="bg-red-100 text-red-800 px-2 py-1 rounded text-sm font-medium">Save 20%</span>
              </div>
            </div>
